...
const Leaf = ({ attributes, children, leaf }) => {
  ...
  if (leaf.center1 || leaf.center2)
    return (
      <span {...attributes} className="relative z-[-1]">
        {leaf.center2 && (
          <span className="absolute top-[-5px] bottom-[-5px] left-[-1px] w-1.5 border-t-2 border-l-2 border-b-2 border-blue-400" />
        )}
        <span className={className}>{children}</span>
        {leaf.center1 && (
          <span className="absolute top-[-5px] bottom-[-5px] right-[-1px] w-1.5 border-t-2 border-r-2 border-b-2 border-blue-400" />
        )}
      </span>
    );
